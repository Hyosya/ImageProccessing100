...
{
    public static class Answer_031
    {
        public static void Solve()
        {
            var img = Cv2.ImRead("imori.jpg");

            var answer1 = Affine(img, 1d, 0d, 0d, 1d, 0d, 0d, 0d, 30d, 30d);
        ...
        private static Mat Affine(Mat img, double a, double b, double c, double d, double tx, double ty, double theta, double dx, double dy)
        {
            // get detriment
            var det = a * d - b * c;
            var resized_width = (int)(img.Width * a + dx);
            var resized_height = (int)(img.Height * d + dy);
            ...
            if (theta != 0)
            {
                ...
                resized_width = img.Width + (int)dx;
                resized_height = img.Height + (int)dy;
            }
